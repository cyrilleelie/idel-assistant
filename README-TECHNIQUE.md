...
### Demarrage rapide via script global (Windows / PowerShell)

Un script `start-all.ps1` a la racine du repo permet de lancer et arreter l'ensemble des services en une commande.

```powershell
.\start-all.ps1              # Lance Docker + backend + frontend web + frontend mobile
.\start-all.ps1 -Gpu         # Idem + stack GPU locale (vLLM, faster-whisper, Kokoro)
.\start-all.ps1 -NoMobile    # Sans Expo mobile
.\start-all.ps1 -Stop        # Arret propre de tous les services lances
```

Les PIDs des services lances sont traces dans `.running-pids.json` (fichier local, ignore par git) pour permettre un arret propre via `-Stop`.

Pour un demarrage manuel service par service (utile en cas de debug), suivre les etapes ci-dessous.
...
Voir [`docs/09-runbook-gpu-deployment.md`](docs/09-runbook-gpu-deployment.md) pour le guide complet.
...
|----------|-------------|
| [`docs/02-architecture-backend.md`](docs/02-architecture-backend.md) | Architecture technique, ADR, modele de donnees |
| [`docs/02b-architecture-tournees.md`](docs/02b-architecture-tournees.md) | Addendum v1.1 : moteur de suggestion de creneaux |
| [`docs/03-architecture-frontend.md`](docs/03-architecture-frontend.md) | Architecture frontend mobile + web |
| [`docs/01-produit-specifications.md`](docs/01-produit-specifications.md) | Product Requirements Document |
| [`docs/cahier-des-charges.md`](docs/cahier-des-charges.md) | Cahier des charges fonctionnel |
| [`docs/05-metier-facturation-ngap.md`](docs/05-metier-facturation-ngap.md) | Regles de facturation NGAP |
| [`docs/06-metier-sesam-vitale.md`](docs/06-metier-sesam-vitale.md) | Roadmap homologation SESAM-Vitale |
...
| [`docs/07-ia-fine-tuning.md`](docs/07-ia-fine-tuning.md) | Guide fine-tuning modele IA |
| [`docs/09-runbook-gpu-deployment.md`](docs/09-runbook-gpu-deployment.md) | Runbook deploiement GPU |
| [`docs/10-methodo-prompts.md`](docs/10-methodo-prompts.md) | Prompts de generation de code |
| [`docs/11-methodo-reviews.md`](docs/11-methodo-reviews.md) | Criteres de review par phase |
| [`docs/04-guide-setup-dev.md`](docs/04-guide-setup-dev.md) | Guide setup developpeur Windows 11 |
