...
from app.application.dtos.invoice_dto import InvoiceDTO, UpdateInvoiceLineDTO
...
class UpdateInvoiceLineUseCase:
    def __init__(
        self,
    ...
    async def execute(
        ...
        if not invoice.is_modifiable:
            raise PermissionError("Impossible de modifier une ligne d'une facture qui n'est pas en brouillon")
        ...
        if dto.supplements is not None:
            line.supplements = dto.supplements

        line.recalculate()
        await self._line_repo.update(line)
