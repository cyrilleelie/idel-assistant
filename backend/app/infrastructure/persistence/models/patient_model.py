...
class PatientModel(Base):
    ...
    # SESAM-Vitale : assuré (si différent du patient)
    insured_nir_encrypted: Mapped[bytes | None] = mapped_column(LargeBinary, nullable=True)
    ...
    sector_id: Mapped[str | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("sectors.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    # SESAM-Vitale : champs en clair (non-identifiants)
    birth_rank: Mapped[int | None] = mapped_column(Integer, nullable=True)
    amo_code: Mapped[str | None] = mapped_column(String(9), nullable=True)
    amo_center: Mapped[str | None] = mapped_column(String(50), nullable=True)
    amc_code: Mapped[str | None] = mapped_column(String(10), nullable=True)
    amc_name: Mapped[str | None] = mapped_column(String(100), nullable=True)
    amc_contract: Mapped[str | None] = mapped_column(String(50), nullable=True)
    exoneration_type: Mapped[str | None] = mapped_column(String(3), nullable=True)
