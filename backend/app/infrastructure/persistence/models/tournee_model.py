...
class TourneeModel(Base):
    ...
    total_duration_hours: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    savings_km: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    savings_minutes: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    num_stops: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    optimization_params = mapped_column(JSONB, nullable=True)
    optimized_at = mapped_column(TIMESTAMP(timezone=True), nullable=True)
