  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 shadow-sm border-l-4 border-l-teal-500 p-4 mt-2 text-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        {date_debut && (
          <span className="ml-auto text-xs text-gray-400">Début {formatDate(date_debut)}</span>
        )}
      </div>
        ...
        {actes.map((a, i) => (
          <div key={i} className="flex items-start gap-2 text-xs">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-700 truncate">{a.description}</div>
              <div className="text-gray-400">{a.frequence} — {a.nb_visites} visites</div>
            </div>
            <div className="text-right shrink-0">
              <div className="font-medium text-gray-700">{(a.montant_estime || 0).toFixed(2)} €</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Observations */}
      {observations && (
        <div className="mb-3 text-xs text-gray-500 bg-gray-50 rounded px-2 py-1.5">
          ℹ {observations}
        </div>
      )}

      {/* Totaux */}
      <div className="flex items-center justify-between text-xs mb-3 border-t border-gray-100 pt-2">
        <span className="text-gray-500">{total_visites} RDV à planifier</span>
      ...
      {pending && (
        <div className="border-t border-gray-100 pt-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-gray-500">Créer les {total_visites} RDV ?</span>
            {!expired && (
