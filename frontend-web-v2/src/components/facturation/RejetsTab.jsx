  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <RefreshCw size={26} className="animate-spin text-slate-300" />
      </div>
    );
  }
    ...
    <div className="space-y-4">
      {error && (
        <div className="flex items-center gap-2 px-4 py-3 text-sm rounded-lg bg-red-50 text-red-700 border border-red-200">
          <AlertCircle size={15} />
          <span>{error}</span>
          <button onClick={() => setError(null)} className="ml-auto hover:text-red-900"><X size={13} /></button>
        </div>
      )}
      ...
      {rejets.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-slate-500 font-medium">Aucune facture rejetée</p>
          <p className="text-sm text-slate-400 mt-1">Toutes les factures sont en ordre.</p>
        </div>
      ) : (
                    ...
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          Soins du {inv.care_date} · <span className="font-medium text-slate-700">{fmt(inv.total_amount)} €</span>
                        </p>
                    ...
                    <div className="space-y-1">
                      {inv.lines.map((line) => (
                        <div key={line.id} className="flex items-center gap-3 text-xs">
                          ...
                          <span className="text-slate-500">{fmt(line.line_total)} €</span>
                        </div>
                      ))}
                    </div>
                  </div>
