...
function StatusBadge({ status }) {
  const map = {
    ...
    exported: { label: 'Exportée', cls: 'bg-amber-100 text-amber-700' },
    transmitted: { label: 'Transmise', cls: 'bg-green-100 text-green-700' },
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12 text-slate-400">
        ...
        Chargement des factures validées…
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {invoices.length === 0 ? (
        <div className="text-center py-10 text-slate-400">
          ...
          <p>Aucune facture validée en attente de FSE.</p>
        </div>
      ) : (
        <>
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-slate-400">
          ...
          <p>Aucune FSE{statusFilter ? ` au statut "${statusFilter}"` : ''}.</p>
        </div>
      ) : (
    ...
    <div className="space-y-6 py-4">
      ...
      {lastResult && step > 1 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-800">
          ...
          <code className="bg-green-100 px-1 rounded">{lastResult.lot_number}</code>.
