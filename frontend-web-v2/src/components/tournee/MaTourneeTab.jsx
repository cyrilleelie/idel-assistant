...
import { ChevronLeft, ChevronRight, Clock, Plus, X, Pencil, CheckCircle2, XCircle, CircleDot, Route } from 'lucide-react';
...
export default function MaTourneeTab({
  ...
  const dayAppts = myAppointments.filter(a => a.dateStr === dateStr);
    ...
    <div className="space-y-6">
      ...
      {slotsWithAppts.length === 0 ? (
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
          ...
          {/* Liste RDV (gauche) + Carte (droite) */}
          <div className="flex flex-col lg:flex-row">
            {/* Créneaux avec RDV */}
            <div className="lg:w-[35%] p-3 space-y-4 overflow-y-auto border-r border-slate-100">
              {slotsWithAppts.map(slot => {
                const slotAppts = dayAppts.filter(a => a.slotId === slot.id);

                return (
                  <div key={slot.id} className="bg-blue-50 border border-blue-200 rounded-lg p-2 flex flex-col gap-2 shadow-sm">
            ...
            <div className="flex-1 p-3">
              <Suspense fallback={<div className="flex items-center justify-center h-[400px] text-sm text-slate-400">Chargement carte...</div>}>
                <TourneeMap
                  ...
                  slotName={`Tournée du ${dayRef.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}`}
                />
              </Suspense>
            </div>
          </div>
