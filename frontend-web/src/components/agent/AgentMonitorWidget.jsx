...
function ProviderRow({ label, name, isLocal, healthy }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-slate-500 w-8 text-xs font-medium">{label}</span>
      <span className="flex-1 text-slate-700 text-xs truncate mx-2">{name}</span>
      ...
      >
        <span
          className={`w-1.5 h-1.5 rounded-full ${
            isOk ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'
          }`}
        />
      ...
      {isExpanded && (
        <div className="absolute bottom-8 right-0 w-72 bg-white border rounded-xl shadow-xl p-4 text-sm">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-slate-900">Statut de l'agent</h3>
            <button
          ...
          {/* Activité du jour */}
          <div className="border-t pt-3 mt-3 flex justify-between text-xs text-slate-500">
          ...
          {health.model_version && (
            <div className="border-t pt-2 mt-3 text-xs text-slate-400 text-center">
