...
export default function MessageFeedback({ message, sessionId, userMessage }) {
  ...
  if (state === 'sent_positive') {
    return <span className="text-xs text-emerald-500 opacity-60 ml-2">Merci !</span>;
  }
  if (state === 'sent_negative') {
    return <span className="text-xs text-slate-400 opacity-60 ml-2">Correction enregistrée</span>;
  }

  // Modal de correction ouverte
  if (state === 'correcting') {
    return (
      <div className="mt-2 border border-slate-200 rounded-lg p-3 bg-slate-50 text-sm">
        <p className="font-medium text-slate-700 mb-1 text-xs">
  ...
  return (
    <div className="flex items-center gap-1.5 mt-0.5 opacity-0 group-hover/msg:opacity-100 transition-opacity duration-150">
      <button
        ...
        className="text-slate-300 hover:text-emerald-500 transition-colors text-xs p-0.5"
        title="Bonne réponse"
      >
