  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 shadow-sm border-l-4 border-l-blue-500 p-4 mt-2 text-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="ml-auto text-xs text-slate-400">{actes_detectes.join(', ')}</span>
      </div>
      ...
      <table className="w-full text-xs mb-3">
        ...
        <tbody>
          {lignes.map((l, i) => (
            <tr key={i} className="border-b border-slate-50">
              ...
              <td className="py-1 text-slate-600 max-w-[120px] truncate">{l.label}</td>
        ...
        <tfoot>
          <tr>
            <td colSpan={2} className="pt-2 font-semibold text-slate-800">Total</td>
      ...
      {pending && (
        <div className="border-t border-slate-100 pt-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-500">Créer le brouillon de facture ?</span>
            {!expired && (
