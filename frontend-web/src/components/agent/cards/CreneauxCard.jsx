  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 shadow-sm border-l-4 border-l-violet-500 p-4 mt-2 text-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="ml-auto text-xs text-slate-400">{care_type || 'soin'} — {duree_minutes} min</span>
      </div>

      {best_slots.length === 0 ? (
        <p className="text-xs text-slate-400 text-center py-2">Aucun créneau disponible</p>
      ) : (
            ...
            >
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-semibold text-slate-800">{formatDate(slot.date)}</span>
                  <span className="text-xs text-slate-600">{slot.start} — {slot.end}</span>
                </div>
                <span className="text-xs text-slate-400">{slot.proximity_label}</span>
              </div>
              <div className="text-right">
                ...
                <span className="text-xs text-slate-300">{Math.round((slot.proximity_score || 0) * 100)}%</span>
              </div>
            </button>
          ))}
        </div>
      ...
      {pending && !expired && selected === null && (
        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
          <span className="text-xs text-slate-400">Cliquer sur un créneau pour confirmer</span>
