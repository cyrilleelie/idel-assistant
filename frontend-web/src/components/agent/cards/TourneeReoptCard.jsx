  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 shadow-sm border-l-4 border-l-amber-500 p-4 mt-2 text-sm">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3 text-center">
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Avant</div>
          <div className="text-sm font-semibold text-slate-700">{avant.total_stops || 0} arrêts</div>
          <div className="text-xs text-slate-500">{avant.distance_km || 0} km</div>
          <div className="text-xs text-slate-400">{avant.duration_min || 0} min</div>
        </div>
        <div className="flex flex-col items-center justify-center">
          <div className="text-emerald-600 text-xs font-semibold">-{economies.distance_km || 0} km</div>
          <div className="text-emerald-600 text-xs">-{economies.duration_min || 0} min</div>
          <span className="text-slate-300 text-lg">→</span>
        </div>
        <div>
          <div className="text-xs text-slate-400 mb-0.5">Après</div>
          <div className="text-sm font-semibold text-emerald-700">{apres.total_stops || 0} arrêts</div>
          <div className="text-xs text-emerald-600">{apres.distance_km || 0} km</div>
          <div className="text-xs text-emerald-500">{apres.duration_min || 0} min</div>
        </div>
      </div>
      ...
      {nouveau_ordre.length > 0 && (
        <div className="mb-3 space-y-0.5">
          <div className="text-xs text-slate-400 mb-1">Nouvel ordre de passages</div>
          {nouveau_ordre.map((s, i) => {
            ...
            return (
              <div key={i} className={`flex items-center gap-2 text-xs py-0.5 ${s.changed ? 'text-amber-700' : 'text-slate-600'}`}>
                <span className="w-5 text-right font-mono text-slate-400">{s.order}.</span>
                {s.changed && <span className="text-amber-400">✦</span>}
                {arrival && <span className="text-slate-500">{arrival}</span>}
              </div>
            );
          })}
        </div>
      ...
      {pending && (
        <div className="border-t border-slate-100 pt-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-slate-500">Valider la réoptimisation ?</span>
            {!expired && (
