            ...
            <div className="divide-y divide-slate-100">
              {[
                ...
                <div key={label} className="flex justify-between items-center px-4 py-2.5 bg-slate-50">
                  ...
                  <span className="text-sm font-medium text-slate-800">{value == null ? '\u2014' : formatCurrency(value)}</span>
                </div>
              ))}
              <div className="flex justify-between items-center px-4 py-3 bg-blue-50 font-semibold">
                ...
                <span className="text-base text-blue-800">{data.total_brut == null ? '\u2014' : formatCurrency(data.total_brut)}</span>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {data.monthly_breakdown.map((mb) => (
                <div key={mb.month} className="bg-slate-50 rounded-lg p-2.5 text-center">
                  ...
                  <p className="text-sm font-semibold text-slate-800 mt-0.5">{mb.total == null ? '\u2014' : formatCurrency(mb.total)}</p>
          ...
          {data.cotisations_estimees && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
              <p className="text-xs text-amber-700 font-medium">
                Estimation cotisations URSSAF (~45%) : {data.cotisations_estimees == null ? '\u2014' : formatCurrency(data.cotisations_estimees)}
              </p>
