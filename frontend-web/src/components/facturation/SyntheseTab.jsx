...
import { formatMonth, formatAmount } from '../../utils/formatting';
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-md px-3 py-2 text-sm">
      ...
      <p className="text-blue-600">{formatAmount(payload[0]?.value)} €</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-blue-200 p-4">
          ...
          <p className="text-2xl font-semibold text-blue-700">{formatAmount(stats?.total_invoiced)} €</p>
        ...
        <div className="bg-white rounded-xl border border-amber-200 p-4">
          ...
          <p className="text-2xl font-semibold text-amber-600">{formatAmount(stats?.total_pending)} €</p>
        ...
        <div className="bg-white rounded-xl border border-emerald-200 p-4">
          ...
          <p className="text-2xl font-semibold text-emerald-600">{formatAmount(stats?.total_paid)} €</p>
        </div>
        <button
        ...
        >
          ...
          <p className={`text-2xl font-semibold ${Number(stats?.total_rejected ?? 0) > 0 ? 'text-red-600' : 'text-slate-400'}`}>
            {formatAmount(stats?.total_rejected)} €
          </p>
        </button>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {stats.top_acts.map((act) => (
                <tr key={act.act_code} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-2.5 text-right font-medium text-slate-800">{formatAmount(act.total)} €</td>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-slate-700">{formatAmount(stats.avg_daily_revenue)} €</p>
            </div>
            <div>
            ...
            {idelStats.map((idel) => (
              <div key={idel.idel_id}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-sm font-medium text-slate-800">{formatAmount(idel.total_invoiced)} €</span>
                </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="px-5 py-2.5 border-b border-slate-100 flex flex-wrap gap-2">
          {[
            ...
            { key: 'validated',   label: 'Validée',     cls: 'bg-emerald-100 text-emerald-700 border-emerald-300' },
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-medium text-slate-800">{formatAmount(inv.total_amount)} €</td>
                    <td className="px-4 py-3"><Badge variant={inv.status} /></td>
